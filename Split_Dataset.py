...
for Class in tqdm(df.iloc[:, 0], unit_scale=True, miniters=1, desc='Splitting Dataset '):
    ...
    for File in Files[:7]:
        shutil.copy(os.path.join('dataset', str(Class), File), os.path.join(
            'Split Dataset', 'Validation', str(Class), File))
    for File in Files[7:]:
        shutil.copy(os.path.join('dataset', str(Class), File), os.path.join(
            'Split Dataset', 'Train', str(Class), File))
    # if len(Files) == 2000:
...
shutil.copy(os.path.join('dataset', 'Reference.csv'),
            os.path.join('Split Dataset', 'Reference.csv'))
